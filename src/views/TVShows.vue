...
<Card v-for="result in results" :key="result.id" :title="result.title" type="TV Show" :image="result.image" :id="result.id"></Card>
</v-lazy>
</div>
</template>
...
export default{
    ...
    search: function () {
        ...
        fetch('https://imdb-api.com/en/API/SearchSeries/k_qy6es45c/'+this.searchValue)
        .then(response => response.json())
        .then(result => {
